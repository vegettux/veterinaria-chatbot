...
def registrar_cliente(nombre: str, dni: str, telefono: str, email: str, direccion: str) -> tuple[bool, str]:
    try:
        ...
        cliente_id = c.fetchone()["id"] if c.description else None
        conn.commit()
        conn.close()
...
def registrar_mascota(
    ...
    mascota_id = c.fetchone()["id"] if c.description else None
    conn.commit()
    conn.close()
...
def get_minutos_ocupados(fecha_str: str, conn=None) -> int:
    ...
    c = conn.cursor()
    c.execute("SELECT COALESCE(SUM(minutos), 0) AS total FROM citas WHERE fecha = %s AND estado = 'confirmada'", (fecha_str,))
    total = c.fetchone()["total"]
...
def listar_citas(fecha_str: Optional[str] = None) -> list[dict]:
    ...
    if fecha_str:
        c.execute("SELECT * FROM citas WHERE fecha = %s ORDER BY fecha", (fecha_str,))
    else:
...
def contar_citas_por_dia(fecha: str):
    ...
    hembras = sum(1 for r in resultados if "hembra" in r["tipo_cirugia"])
    grandes = sum(1 for r in resultados if "30" in r["tipo_cirugia"] or "40" in r["tipo_cirugia"])
